...
void close_server_connection() {
    ...
    // shutdown sockets
    int shut = shutdown(serverSocket, SHUT_RDWR);
    if (shut == -1) {
        perror("shutdown failed \n");
    }

    // close sockets
    int closed = close(serverSocket);
    if (closed == -1) {
        perror("close failed \n");
    }
...
int connect_to_server(const char *host, const char *port) {
    printf("hello there"); // print out the message?

    int socket_temp = socket(AF_INET, SOCK_STREAM, 0);
    if (socket_temp == -1) {
    ...
    int get_addr_result = getaddrinfo(host, port, &hints, &res);
    ...
    int connect_res = connect(socket_temp, res->ai_addr, res->ai_addrlen);
    if (connect_res == -1) {
